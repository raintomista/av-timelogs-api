...
module.exports = function(req,res,next){
    ...
    Admin.find({$or: [{username: req.params.data.username},{email: req.params.data.email}]}).exec(function(err,results){
        if(!err){
            if(results.length > 0){
                res.send(400,{code: vars.CODE_BAD_REQUEST , message:"Username/Email already exists"});
            } else {
                Admin.create(data);
                  res.send(200,{code: vars.CODE_SUCCESS, message:"Admin has been created",
                    data:{
