...
module.exports = function (req, res, next) {
	...
	function findUser(username) {
		return User.findOne({username: username}, {username: 1, status: 1})
			.then(user => {
				return user;
			})
			.catch(err => {
	...
	function saveOffset(userId){
		...
		newOffset.save()
			.then(offset => {
				...
				let update = { status: 1, _offset: newOffset._id  };
				updateUserStatus(query, update, 0, 'Successfully timed in in Offset Mode');
			})
			.catch(err => {
				res.send(500, {
	...
	function updateUserStatus(query, update, lateHrs, message){
