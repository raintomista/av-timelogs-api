...
cron.scheduleJob({hour: 12, minute: 17, dayOfWeek: [1,2,3,4,5]}, function(){
    console.log('NOON MAILER');
    timeInAlert();
});

cron.scheduleJob({hour: 0, minute: 10, dayOfWeek: [1,2,3,4,5]}, function(){
...
timeInAlert = function(){
        User.find({status: 0}, {email: 1, firstName: 1, lastName: 1}, function(err, results){
